...
//prestasi
  else if($akh==md5('addprestasi')){ $c_prestasi=random(9);
    $smk->addprestasi($con,$c_prestasi,$_POST['nama'],$_POST['username'],$_POST['password']);
  }
  else if($akh==md5('editprestasi')){
    $smk->editprestasi($con,$_POST['c_prestasi'],$_POST['nama'],$_POST['username'],$_POST['password']);
  }
  else if($akh==md5('hapusprestasi')){
    $dd=mysqli_fetch_array(mysqli_query($con,"SELECT * from prestasi where c_prestasi='$_GET[q]' "));
    if($dd['foto']!=NULL){
        unlink('../../prestasi/'.$dd['foto'].'');
    }
    $smk->hapusprestasi($con,$_GET['q']);
  }
//orangtua
